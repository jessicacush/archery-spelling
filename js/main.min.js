...
var hangmanSpellingGame = (function(){
  ...
  if (gameData.activity.settings.realKeyboard) {
    ...
    if (keyLetter === nextNeeded) {
      correctKey = true;
      currentAnswer[currentAnswerCount] = questionTextArray[currentAnswerCount];
      currentAnswerCount++;
      if (audio && audio['correct']) audio['correct'].play();
    } else {
      if (audio && audio['wrong']) audio['wrong'].play();
      if (uiCounter && uiCounter.reduce) uiCounter.reduce();
    }

  } else {
    if (key.classList.contains('correct-key') || key.classList.contains('wrong-key')) return;
  ...
  if (currentAnswerCount === questionTextLength) {
    answerCorrect();
  } else if (!correctKey && uiCounter && uiCounter.amount <= 0) {
    answerWrong(false);
  }
};
  ...
  if (currentAnswer.join('') == questionText) {
    var delay = 1000;
    var level = gameData.activity.level[gameData.questionNumber] || {};
    var audioId = level.audio;

    if (audioId && audio[audioId] && typeof audio[audioId]._duration === "number") {
      delay = Math.max(1000, audio[audioId]._duration * 1000);
    }
    ...
    setTimeout(function () {
      ...
      setTimeout(function () {
        var finishedNoBank = (gameData.questionNumber == gameData.activity.level.length && !gameData.activity.bank.useBank);
        var finishedBank   = (gameData.questionCount == gameData.activity.bank.numberOfQuestionPerGame && gameData.activity.bank.useBank);

        if (finishedNoBank || finishedBank) {
          endScreen.init();
...
var answerWrong = function (hint) {
  ...
  if (uiCounter.amount == 0 || currentAnswer.join('') == questionText) {
    ...
    var delay = 0;
    if (gameData.activity.level[gameData.questionNumber].audio &&
        typeof gameData.activity.level[gameData.questionNumber].audio != "undefined") {
