...
var hangmanSpellingGame = (function(){
	...
	var getHint = function() {
		if(hintCount != 3){
			var hintLetters = [],
				hintLetter = "",
				allKeys = document.getElementsByClassName("hangman-keyboard-key");
				var hintLetters = [];
			
			// BUG: This adds ALL letters, not filtering out already revealed ones
			for(var i=0; i<questionTextLength; i++){
				if (!currentAnswer[i] || currentAnswer[i] === "_") {
					hintLetters.push(questionTextArray[i]);
				}
			}
			hintLetters = Array.from(new Set(hintLetters));
			var hintLetter = "";
			
			if( gameData.activity.settings.realKeyboard ){
				hintLetter = questionTextArray[currentAnswerCount];
				currentAnswer[currentAnswerCount] = hintLetter;
				currentAnswerCount++;
			} else {
				if (hintLetters.length === 0) return;
				hintLetter = hintLetters[random.number(hintLetters.length)-1];
				...
				for(var i=0; i<questionTextLength; i++){
					if(hintLetter === questionTextArray[i]){	
						currentAnswer[i] = questionTextArray[i];
						currentAnswerCount++;
					}
				}
			}
			...
			if(uiCounter.amount == 1){
				lastChance = true;
			}
			
			lettersLeft = new Set(hintLetters);
			lettersLeft = Array.from(lettersLeft);
			
			if(lettersLeft.length > 1 && lastChance) {
				answerWrong(true);
			} else if(hintCount != 3 || hintCount != questionTextLength) {
				answerCorrect(true);
			} else {
				answerWrong(true);
			}
		...
		if(hintCount == 2){
			hintButtonElement.style.opacity = 0;
		}
	}
	...
	var checkAnswer = function(key) {
		...
		if( gameData.activity.settings.realKeyboard ){
			...
			if( !key.classList.contains('correct-key') && !key.classList.contains('wrong-key') && !key.classList.contains('hint-key') ){
				var correctKey = false;
				
				for(var i=0; i<questionTextLength; i++){
					// BUG: Another case sensitive comparison
					if(keyLetter === (questionTextArray[i] || "").toLowerCase()){
						correctKey = true;
						currentAnswer[i] = questionTextArray[i];
						currentAnswerCount++;
